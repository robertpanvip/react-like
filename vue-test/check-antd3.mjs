...
console.log('REACT_FORWARD_REF_TYPE:', mod.REACT_FORWARD_REF_TYPE.toString())
console.log('Button.$typeof matches:', antd.Button.$typeof === mod.REACT_FORWARD_REF_TYPE)

// Check if Button is wrapped by createElement
console.log('\n=== createElement with Button ===')
const el2 = mod.createElement(antd.Button, {type: 'primary'}, 'Click Me')
console.log('el2 type is function:', typeof el2.type === 'function')
console.log('el2 type.$typeof:', el2.type?.$typeof?.toString())
console.log('el2 type.$typeof === REACT_FORWARD_REF_TYPE:', el2.type?.$typeof === mod.REACT_FORWARD_REF_TYPE)

// Verify toVNode works
console.log('\n=== toVNode ===')
try {
    const vnode = mod.toVNode(el2)
    console.log('VNode result:', vnode)
    console.log('VNode type:', typeof vnode.type === 'function' ? 'function' : vnode.type)
} catch (e) {
    console.log('toVNode error:', e.message)
}